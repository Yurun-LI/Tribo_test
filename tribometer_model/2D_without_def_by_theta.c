...
double cal_F_ij(int i, int j, double Rho_c_bar[N + 1][M + 1], double H[N + 1][M + 1], double Eta_bar[M + 1][N + 1], double theta[N + 1][M + 1])
{
    ...
    Ax_ij = 1 / (2 * DX * DX) * ((Epsilon_ip1j + Epsilon_ij) * g_theta_m1(theta[j][i + 1]) - (Epsilon_ip1j + 2 * Epsilon_ij + Epsilon_im1j) * g_theta_m1(theta[j][i]) + (Epsilon_im1j + Epsilon_ij) * g_theta_m1(theta[j][i - 1]));
    Ay_ij = 1 / (2 * DY * DY) * ((Epsilon_ijp1 + Epsilon_ij) * g_theta_m1(theta[j + 1][i]) - (Epsilon_ijp1 + 2 * Epsilon_ij + Epsilon_ijm1) * g_theta_m1(theta[j][i]) + (Epsilon_ijm1 + Epsilon_ij) * g_theta_m1(theta[j][i - 1]));
...
double cal_N_ij_kl(int i, int j, int k, int l, double Rho_c_bar[N + 1][M + 1], double H[N + 1][M + 1], double theta[N + 1][M + 1])
{
    ...
    N1 = (theta[j][i] * H[j][i]) * Rc;
    N2 = (theta[j][i] * Rho_c_bar[j][i]) * De;
...
double cal_J0(int i, int j, double Rho_c_bar[N + 1][M + 1], double H[N + 1][M + 1], double Eta_bar[N + 1][M + 1], double theta[N + 1][M + 1])
{
    ...
    Ax0 = 1 / (2 * DX * DX) * (g_theta_m1(theta[j][i + 1]) * (M2 + M3) - g_theta_m1(theta[j][i]) * (M1 + M3 * 2 + M2) + g_theta_m1(theta[j][i - 1]) * (M1 + M3) + (Epsilon_ip1j + Epsilon_ij));
    Ay0 = 1 / (2 * DY * DY) * (g_theta_m1(theta[j + 1][i]) * (M5 + M3) - g_theta_m1(theta[j][i]) * (M4 + M3 * 2 + M5) + g_theta_m1(theta[j - 1][i]) * (M4 + M3));
...
double cal_J1(int i, int j, double Rho_c_bar[N + 1][M + 1], double H[N + 1][M + 1], double Eta_bar[N + 1][M + 1], double theta[N + 1][M + 1])
{
    ...
    Ax1 = 1 / (2 * DX * DX) * (g_theta_m1(theta[j][i + 1]) * (M2 + M3) - g_theta_m1(theta[j][i]) * (M1 + M3 * 2 + M2) + g_theta_m1(theta[j][i - 1]) * (M1 + M3) + (Epsilon_im1j + Epsilon_ij));
    Ay1 = 1 / (2 * DY * DY) * (g_theta_m1(theta[j + 1][i]) * (M5 + M3) - g_theta_m1(theta[j][i]) * (M4 + M3 * 2 + M5) + g_theta_m1(theta[j - 1][i]) * (M4 + M3));
...
double cal_J2(int i, int j, double Rho_c_bar[N + 1][M + 1], double H[N + 1][M + 1], double Eta_bar[N + 1][M + 1], double theta[N + 1][M + 1])
{
    ...
    Ax2 = 1 / (2 * DX * DX) * (g_theta_m1(theta[j][i + 1]) * (M2 + M3) - g_theta_m1(theta[j][i]) * (M1 + M3 * 2 + M2) + g_theta_m1(theta[j][i - 1]) * (M1 + M3));
    Ay2 = 1 / (2 * DY * DY) * (g_theta_m1(theta[j + 1][i]) * (M5 + M3) - g_theta_m1(theta[j][i]) * (M4 + M3 * 2 + M5) + g_theta_m1(theta[j - 1][i]) * (M4 + M3) + (Epsilon_ijp1 + Epsilon_ij));
...
double cal_J3(int i, int j, double Rho_c_bar[N + 1][M + 1], double H[N + 1][M + 1], double Eta_bar[N + 1][M + 1], double theta[N + 1][M + 1])
{
    ...
    Ax3 = 1 / (2 * DX * DX) * (g_theta_m1(theta[j][i + 1]) * (M2 + M3) - g_theta_m1(theta[j][i]) * (M1 + M3 * 2 + M2) + g_theta_m1(theta[j][i - 1]) * (M1 + M3));
    Ay3 = 1 / (2 * DY * DY) * (g_theta_m1(theta[j + 1][i]) * (M5 + M3) - g_theta_m1(theta[j][i]) * (M4 + M3 * 2 + M5) + g_theta_m1(theta[j - 1][i]) * (M4 + M3) + (Epsilon_ijm1 + Epsilon_ij));
...
double cal_J4(int i, int j, double Rho_c_bar[N + 1][M + 1], double H[N + 1][M + 1], double Eta_bar[N + 1][M + 1], double theta[N + 1][M + 1])
{
    ...
    Ax4 = 1 / (2 * DX * DX) * (g_theta_m1(theta[j][i + 1]) * (M2 + M3) - g_theta_m1(theta[j][i]) * (M1 + M3 * 2 + M2) + g_theta_m1(theta[j][i - 1]) * (M1 + M3) - (Epsilon_ip1j + Epsilon_im1j + 2 * Epsilon_ij));
    Ay4 = 1 / (2 * DY * DY) * (g_theta_m1(theta[j + 1][i]) * (M5 + M3) - g_theta_m1(theta[j][i]) * (M4 + M3 * 2 + M5) + g_theta_m1(theta[j - 1][i]) * (M4 + M3) - (Epsilon_ijp1 + Epsilon_ijm1 + 2 * Epsilon_ij));
...
int main(int argc, const char *argv[])
{
    ...
    E_dash = 2 / ((1 - pow(v1, 2)) / E1 + (1 - pow(v2, 2)) / E2);
    //initialize nodes
    init_param(&node);
    ...
    do
    {
        ...
        for (int j = 0; j <= N; j++)
        {
            for (int i = 0; i <= M; i++)
            {
                diff_sqrt_theta += fabs(theta_new[j][i] - theta_pervious[j][i]);
            }
        }
        diff_sqrt_theta = diff_sqrt_theta / (M * N);
        printf("the error is :\t%.10lf\n", diff_sqrt_theta);
    } while (diff_sqrt_theta > Err_thetaMin || theta_loop < 1000);

    //save data
    fp1 = fopen("/Users/liyurun/Desktop/tribometer_data/data_for_20210701/theta.csv", "w+");
    for (int j = 0; j<N+1; j++){
        for (int i = 0; i< M+1; i++){
    ...
    fclose(fp1);
    fp2 = fopen("/Users/liyurun/Desktop/tribometer_data/data_for_20210701/pressure.csv", "w+");
    for (int j = 0; j<N+1; j++){
        for (int i = 0; i< M+1; i++){
