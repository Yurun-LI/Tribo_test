...
#define R 7.5e-3
#define Omega 1.0e-3
#define M 20                //maximum node in x-axis
#define N 20                //maximum node in y-axis
...
// //De = dH[i,j]/d(g_theta)[k,l]
// double cal_delta_ij(int i, int j, double P[N+1][M+1])
// {
//     double delta_ij = 0.0;
//     int m,n;
//     double D_mn;
...
//             n = abs(j-yj);
//             D_mn = cal_D_mn(m, n);
//             delta_ij += (2*P_h/(PI*E_dash)) * P[yi][xi];
//         }
//     }
//     return delta_ij;
// }
...
// delta[i.j]
double cal_delta_ij(int i, int j, double theta[N + 1][M + 1])
{
    double p_kl, delta_ij = 0.0;
    for (int l = 0; l <= N; l++)
    {
        for (int k = 0; k <= M; k++)
        {
            p_kl = pressure(theta[l][k]); // deflection of pressure[k,l] made to node[i,j]
            if (i == k && j == l) continue;
            delta_ij = delta_ij + (1 / (PI * E_dash)) * p_kl * (dx * dy) / sqrt(pow((abs(k - i) * dx), 2) + pow((abs(l - j) * dy), 2));
        }
    }
    return delta_ij;
}
// De[ij,kl] = dH[i,j] / d(g_theta)[k,l]
double cal_De_ij_kl(int i, int j, int k, int l, double theta[N + 1][M + 1])
{
    if (i==k && j==l)
    {
        return 0.0;
    }
    return (1 / (PI * E_dash)) * (dx * dy) / sqrt(pow((abs(k - i) * dx), 2) + pow((abs(l - j) * dy), 2)) * (beta);
}
...
void update_param(struct Nodes *node)
{
    ...
    for (int j = 1; j < N; j++)
    {
        for (int i = 1; i < M; i++)
        {
            ...
            delta = cal_delta_ij(i, j, node->theta);
            node->Delta[j][i] = delta / (b * b / R);
            node->H[j][i] = node->H0 + node->HS[j][i] + node->Delta[j][i];
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    printf_param(node.P);
    //save data
    fp1 = fopen("/Users/liyurun/Desktop/tribometer_data/data_for_2D_param_rho_eta_def_by_theta/node_20_param_Rho_Eta_theta.csv", "w+");
    for (int j = 0; j < N + 1; j++)
    {
        for (int i = 0; i < M + 1; i++)
        {
    ...
    fclose(fp1);
    fp2 = fopen("/Users/liyurun/Desktop/tribometer_data/data_for_2D_param_rho_eta_def_by_theta/node_20_param_Rho_Eta_pressure.csv", "w+");
    for (int j = 0; j < N + 1; j++)
    {
        for (int i = 0; i < M + 1; i++)
        {
    ...
    fclose(fp2);
    fp1 = fopen("/Users/liyurun/Desktop/tribometer_data/data_for_2D_param_rho_eta_def_by_theta/node_20_param_Rho_Eta_filmthickness.csv", "w+");
    for (int j = 0; j < N + 1; j++)
    {
        for (int i = 0; i < M + 1; i++)
        {
            fprintf(fp1, "%.10lf%s", node.H[j][i] * (b * b / R),
                    (i < M + 1 - 1 ? "," : ""));
        }
        fprintf(fp1, "\n");
    }
    fclose(fp1);
    fp2 = fopen("/Users/liyurun/Desktop/tribometer_data/data_for_2D_param_rho_eta_def_by_theta/node_20_param_Rho_Eta_delta.csv", "w+");
    for (int j = 0; j < N + 1; j++)
    {
        for (int i = 0; i < M + 1; i++)
        {
            fprintf(fp2, "%.10lf%s", node.Delta[j][i] * (b * b / R),
                    (i < M + 1 - 1 ? "," : ""));
        }
        fprintf(fp2, "\n");
    }
